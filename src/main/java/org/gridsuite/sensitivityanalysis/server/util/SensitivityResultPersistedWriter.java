...
public class SensitivityResultPersistedWriter implements SensitivityResultWriter, AutoCloseable {
     ...
     * We choose here to explicitly wait for both pollers to finish. It's a choice for simplicity and symmetricity.
     * The resulting exception (if any) priories InterruptedException over ExecutionException and composes it with all the suppressed exceptions.
     */
    ...
    private Exception waitForSensitivityException() {
        try {
    ...
    private Exception waitForContingencyException() {
        try {
