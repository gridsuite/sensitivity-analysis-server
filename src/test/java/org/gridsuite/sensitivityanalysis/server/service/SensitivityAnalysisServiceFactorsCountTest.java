...
class SensitivityAnalysisServiceFactorsCountTest {
    ...
    void testAllTypes() {
        ...
        assertEquals(36L, result.resultCount(),
                "(injections set: 2 mon. × (1 base + 1 cont.)) + (injections: 1 mon. × 2 var. × (1 base + 2 cont.)) + (hvdc: 1 mon. × 1 var. × 1 base) + (pst: 1 mon. × 1 var. × 1 base) + (nodes: "
                        + "2 mon. × 3 var. × 2 emp. fact. × (1 base + 1 cont.)) = 36");
    }
    ...
    void testThrowExceptionOnMissingContingencies() {
        ...
        SensitivityAnalysisParametersInfos parameters = createParameters(
                List.of(new SensitivityInjectionsSet(List.of(BRANCH1_UUID), List.of(), null, List.of(CONTINGENCY1_UUID), true)),
                null, null, null, null
        );
